...
# CSI E2E Test
echo "CSI E2E Test"
# Try to ignore csi test
sh scripts/ci/csi-e2e-test.sh || exit 1

# DatenLord Perf Test
# TODO: avoid to re-create cluster
echo "DatenLord Perf Test"
kind delete cluster || exit 1
kind create cluster --config /tmp/kind-config.yaml || exit 1
kind load docker-image ${DATENLORD_IMAGE} || exit 1
echo "Deploy DatenLord to K8S datenlord-perf-test.sh"
